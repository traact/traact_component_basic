...
class EstimatePose : public Component {
 public:
    ...
    bool tryTracking(size_t mea_idx,
                     const spatial::Position2DList &points2d,
                     const spatial::Position3DList &points3d,
                     const traact::vision::CameraCalibration &calibration,
                     spatial::Pose6D &output,
                     spatial::Position2DList &output_points) {
    ...
    bool testAllCombinations(size_t mea_idx,
                             const spatial::Position2DList &points2d,
                             const spatial::Position3DList &points3d,
                             const traact::vision::CameraCalibration &calibration,
                             spatial::Pose6D &output,
        ...
        if (result) {
            ...
            if (min_error > maxError_) {
                SPDLOG_INFO("{3}: found pose but rejected, final error bigger then max error {2}: {0} in {1} tests",
                            min_error,
                            num_tests,
                            maxError_,
                            getName());
            ...
            SPDLOG_INFO("{0}: {1}: no pose found final error: {2} in {3} tests",
                        getName(),
                        mea_idx,
                        min_error,
                        num_tests);
    ...
    bool recursiveTestCombination(size_t mea_idx,
                                  int current_point,
                                  int max_test,
                                  const spatial::Position2DList &points2d,
                                  const spatial::Position3DList &points3d,
                                  const traact::vision::CameraCalibration &calibration,
                                  const std::vector<std::vector<std::pair<int, double> > > &distances_3d,
                                  const std::vector<std::vector<std::pair<int, double> > > &distances_2d,
                                  spatial::Pose6D &output,
                                  spatial::Position2DList &output_points,
        ...
        if (current_point == 0) {
            ...
            for (int first_point_idx = 0; first_point_idx < points2d.size(); ++first_point_idx) {
                ...
                bool result = recursiveTestCombination(mea_idx,
                                                       1,
                                                       max_test,
                                                       points2d,
                                                       points3d,
    ...
    bool tryClusterCombinations(size_t mea_idx,
                                const spatial::Position2DList &points2d,
                                const spatial::Position3DList &points3d,
                                const traact::vision::CameraCalibration &calibration,
                                spatial::Pose6D &output,
...
//                }
    ...
    bool processTimePoint(buffer::ComponentBuffer &data) override {
        ...
        const auto &points2d = data.getInput<spatial::Position2DListHeader>(0);
        const auto &points3d = data.getInput<spatial::Position3DListHeader>(1);
        const auto &calibration = data.getInput<CameraCalibrationHeader>(2);

        auto &output = data.getOutput<traact::spatial::Pose6DHeader>(0);
        auto &output_points = data.getOutput<traact::spatial::Position2DListHeader>(1);
...
//            }
        ...
        if (!result) {
            //Timestamp ts_start = now();
            //bool result1 = testAllCombinations(data.GetMeaIdx(), points2d, points3d, calibration, output, output_points);
            //TimeDuration dur1 = now()- ts_start;

            Timestamp ts_start2 = now();
            result = tryClusterCombinations(data.getTimestamp().time_since_epoch().count(),
                                            points2d,
                                            points3d,
                                            calibration,
                                            output,
